...
TEST_TEAR_DOWN(observer_unsubscribe) { /* Cleanup after every test */ }
...
TEST(
  ...
  static observer_cb_t expected_subsribtion[SYBSCRIPTION_CALBACKS_TABLE_SIZE] =
      {NULL};
  // Given
  ...
  // Then
  TEST_ASSERT_EQUAL_MEMORY_ARRAY(expected_subsribtion, subscribtion,
                                 memory_obj_byte_len,
...
#define LAST_CB_TAB_ELEMENT (SYBSCRIPTION_CALBACKS_TABLE_SIZE - 1)
  ...
  for (uint8_t i = 0; i < SYBSCRIPTION_CALBACKS_TABLE_SIZE; i++) {
    expected_subsribtion[i] =
        (observer_cb_t)(&mock_fun_1 + (FUN_ADR_OFFSET * i));
  }
  unsubscr_fun = expected_subsribtion[LAST_CB_TAB_ELEMENT - 1];
  expected_subsribtion[LAST_CB_TAB_ELEMENT - 1] =
      expected_subsribtion[LAST_CB_TAB_ELEMENT];
  expected_subsribtion[LAST_CB_TAB_ELEMENT] = NULL;
  // Given
  for (uint8_t i = 0; i < SYBSCRIPTION_CALBACKS_TABLE_SIZE; i++) {
    subscribe(subscribtion, (observer_cb_t)(mock_fun_1 + (FUN_ADR_OFFSET * i)),
              SYBSCRIPTION_CALBACKS_TABLE_SIZE);
  }
  // When
...
#define UNSUBSCR_FUNC_TAB_ELEMENT (LAST_CB_TAB_ELEMENT - 2)
  ...
  expected_subsribtion[UNSUBSCR_FUNC_TAB_ELEMENT] =
      expected_subsribtion[UNSUBSCR_FUNC_TAB_ELEMENT + 1];
