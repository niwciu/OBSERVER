...
static void mock_dirty_fun_with_arg(uint8_t state)
{
    (void)state;
}
...
TEST(observer_subscribe_with_u8_arg, GivenSubscriptionTableEmptyWhenSubscribeMockFun1ThenFirstElementEqualsMockFun1AndRestAreEmpty)
{
    ...
    subscribe_u8(subscription, mock_fun_with_arg_1, SUBSCRIPTION_CALLBACKS_TABLE_SIZE);
...
TEST(observer_subscribe_with_u8_arg, GivenSubscriptionTableEmptyWhenSubscribeMockFun1ThenMockFun2ThenCorrectOrderInTable)
{
    ...
    subscribe_u8(subscription, mock_fun_with_arg_1, SUBSCRIPTION_CALLBACKS_TABLE_SIZE);
    subscribe_u8(subscription, mock_fun_with_arg_2, SUBSCRIPTION_CALLBACKS_TABLE_SIZE);
...
TEST(observer_subscribe_with_u8_arg, GivenSubscriptionTableEmptyWhenSubscribe3FunctionsThenFirst3ElementsSetAccordingToOrder)
{
    ...
    subscribe_u8(subscription, mock_fun_with_arg_1, SUBSCRIPTION_CALLBACKS_TABLE_SIZE);
    subscribe_u8(subscription, mock_fun_with_arg_2, SUBSCRIPTION_CALLBACKS_TABLE_SIZE);
    subscribe_u8(subscription, mock_fun_with_arg_3, SUBSCRIPTION_CALLBACKS_TABLE_SIZE);
...
TEST(observer_subscribe_with_u8_arg, GivenFullTableWhenSubscribeAnotherCallbackThenReturnTableFullError)
{
    ...
    TEST_ASSERT_EQUAL(CALLBACK_ERROR_TABLE_FULL, subscribe_u8(subscription, mock_fun_with_arg_1, SUBSCRIPTION_CALLBACKS_TABLE_SIZE));
}
...
TEST(observer_subscribe_with_u8_arg, GivenNotFullTableWhenSubscribeThenReturnCallbackSubscribedOk)
{
    ...
    TEST_ASSERT_EQUAL(CALLBACK_SUBSCR_OK, subscribe_u8(subscription, mock_fun_with_arg_1, SUBSCRIPTION_CALLBACKS_TABLE_SIZE));
}
