...
TEST_GROUP(observer_subscribe_with_arg);

TEST_SETUP(observer_subscribe_with_arg)
{
    clear_subscription_table();
    mock_reset_mock_fun_with_arg_counters();
}

TEST_TEAR_DOWN(observer_subscribe_with_arg)
{
}

TEST(observer_subscribe_with_arg, GivenSubscriptionTableEmptyWhenSubscribeMockFun1ThenFirstElementEqualsMockFun1AndRestAreEmpty)
{
    static uint32_t memory_obj_byte_len = sizeof(observer_cb_arg_t);
    static observer_cb_arg_t expected_subscription[SUBSCRIPTION_CALLBACKS_TABLE_SIZE] = {NULL};
...
TEST(observer_subscribe_with_arg, GivenSubscriptionTableEmptyWhenSubscribeMockFun1ThenMockFun2ThenCorrectOrderInTable)
{
    static uint32_t memory_obj_byte_len = sizeof(observer_cb_arg_t);
    static observer_cb_arg_t expected_subscription[SUBSCRIPTION_CALLBACKS_TABLE_SIZE] = {NULL};
    expected_subscription[0] = mock_fun_with_arg_1;
...
TEST(observer_subscribe_with_arg, GivenSubscriptionTableEmptyWhenSubscribe3FunctionsThenFirst3ElementsSetAccordingToOrder)
{
    static uint32_t memory_obj_byte_len = sizeof(observer_cb_arg_t);
    static observer_cb_arg_t expected_subscription[SUBSCRIPTION_CALLBACKS_TABLE_SIZE] = {NULL};
    expected_subscription[0] = mock_fun_with_arg_1;
...
TEST(observer_subscribe_with_arg, GivenFullTableWhenSubscribeAnotherCallbackThenReturnTableFullError)
{
    ...
    TEST_ASSERT_EQUAL(CALLBACK_ERROR_TABLE_FULL, subscribe_enter_exit(subscription, mock_fun_with_arg_1, SUBSCRIPTION_CALLBACKS_TABLE_SIZE));
}

TEST(observer_subscribe_with_arg, GivenNotFullTableWhenSubscribeThenReturnCallbackSubscribedOk)
{
    ...
    TEST_ASSERT_EQUAL(CALLBACK_SUBSCR_OK, subscribe_enter_exit(subscription, mock_fun_with_arg_1, SUBSCRIPTION_CALLBACKS_TABLE_SIZE));
}

TEST(observer_subscribe_with_arg, GivenEmptyTableWhenSubscribeSameCallback3TimesThenOnlyFirstSlotIsFilled)
{
    static uint32_t memory_obj_byte_len = sizeof(observer_cb_arg_t);
    static observer_cb_arg_t expected_subscription[SUBSCRIPTION_CALLBACKS_TABLE_SIZE] = {NULL};
    expected_subscription[0] = mock_fun_with_arg_1;
